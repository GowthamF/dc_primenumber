...
func NotifyLearnerNodeProcess(url *string, outcome *models.PrimeNumbersValidationMessage) {
	json_data, err := json.Marshal(outcome)
	if err != nil {
		go sidecar.Log(*models.NodeId + err.Error())
	}
	...
	req, err := http.NewRequest("POST", *url+"/notifyLearner", bytes.NewBuffer(json_data))
	...
	if resp != nil {
		if resp.StatusCode == 200 {
			log.Println("Sent")
			go sidecar.Log(*models.NodeId + " Learner has been notified")

		} else {
			log.Println("Failed")
			go sidecar.Log(*models.NodeId + " Learner notified failed")
		}
	}
}
